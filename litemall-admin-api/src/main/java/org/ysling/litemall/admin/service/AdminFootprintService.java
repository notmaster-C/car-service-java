...
import org.ysling.litemall.admin.model.footprint.body.FootprintListBody;
...
public class AdminFootprintService extends FootprintServiceImpl {
    ...
    @Cacheable(sync = true)
    public List<LitemallFootprint> querySelective(FootprintListBody body) {
        QueryWrapper<LitemallFootprint> wrapper = startPage(body);
        if (StringUtils.hasText(body.getUserId())) {
            wrapper.eq(LitemallFootprint.USER_ID , body.getUserId());
        }
        if (StringUtils.hasText(body.getGoodsId())) {
            wrapper.eq(LitemallFootprint.GOODS_ID , body.getGoodsId());
        }
